...
void Testbench::do_sobel() {
  int x, y, v, u, i;        // for loop counter
  unsigned char R, G, B; // color of R, G, B
  int adjustX, adjustY, xBound, yBound;
  int result_r, result_g, result_b;
  unsigned char R_buffer[3][256], G_buffer[3][256], B_buffer[3][256]; 

  o_rst.write(false);
  o_rst.write(true);
  for (y = 0; y < 2; y++) {
    for (x = 0; x != width; x++) {
      R_buffer[y][x] = *(source_bitmap + bytes_per_pixel * (width * y + x) + 2);
      G_buffer[y][x] = *(source_bitmap + bytes_per_pixel * (width * y + x) + 1);
      B_buffer[y][x] = *(source_bitmap + bytes_per_pixel * (width * y + x) + 0);
      wait(1);
    }
  }
  for (x = 0; x != width; x++) {
    R_buffer[2][x] = *(source_bitmap + bytes_per_pixel * (width * 2 + x) + 2);
    G_buffer[2][x] = *(source_bitmap + bytes_per_pixel * (width * 2 + x) + 1);
    B_buffer[2][x] = *(source_bitmap + bytes_per_pixel * (width * 2 + x) + 0);
    for (v = -1; v != 2; v++) {
      for (u = -1; u != 2; u++) {
        if (x + u >= 0 && x + u < width && v != -1) {
          R = R_buffer[v][x + u];
          G = G_buffer[v][x + u];
          B = B_buffer[v][x + u];
        }
    ...
    *(target_bitmap + bytes_per_pixel * x + 2) = result_r;
    *(target_bitmap + bytes_per_pixel * x + 1) = result_g;
    *(target_bitmap + bytes_per_pixel * x + 0) = result_b;
  }
  for (y = 1; y != height; ++y) {
    for (x = 0; x != width; ++x) {
    ...
    for (i = 0; i != width; ++i) {
      if (y % 3 == 1) {
        R_buffer[0][i] = *(source_bitmap + bytes_per_pixel * (width * (y + 2) + i) + 2);
        G_buffer[0][i] = *(source_bitmap + bytes_per_pixel * (width * (y + 2) + i) + 1);
        B_buffer[0][i] = *(source_bitmap + bytes_per_pixel * (width * (y + 2) + i) + 0);
      }
      else if (y % 3 == 2) {
        R_buffer[1][i] = *(source_bitmap + bytes_per_pixel * (width * (y + 2) + i) + 2);
        G_buffer[1][i] = *(source_bitmap + bytes_per_pixel * (width * (y + 2) + i) + 1);
        B_buffer[1][i] = *(source_bitmap + bytes_per_pixel * (width * (y + 2) + i) + 0);
      }
      else {
        R_buffer[2][i] = *(source_bitmap + bytes_per_pixel * (width * (y + 2) + i) + 2);
        G_buffer[2][i] = *(source_bitmap + bytes_per_pixel * (width * (y + 2) + i) + 1);
        B_buffer[2][i] = *(source_bitmap + bytes_per_pixel * (width * (y + 2) + i) + 0);
      }
      wait(1);
    }
  }
